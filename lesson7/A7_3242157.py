...
def add_index(data, name, amount):

    highest_index = -1;

    for entry in data:
        if entry[0] > highest_index: highest_index = entry[0];

    new_index = highest_index + 1;

    data.append([new_index, name, amount]);
...
if mode == 4:
    print("Geben Sie den Namen des Kontoinhabers an.");
    name = input(" Ihre Eingabe: ");

    print("Geben Sie den initialen Kontostand an.");
    amount = float(input(" Ihre Eingabe: "));

    data = add_index(data, name, amount);

    write_data(data);

    print("Das Konto wurde angelegt");
